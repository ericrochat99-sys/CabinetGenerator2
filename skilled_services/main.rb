...
module SkilledServices
  ...
  PLUGIN_NAME = "Euro Cabinet Generator" unless const_defined?(:PLUGIN_NAME)
  ...
  TOE_KICK_SKU           = 'PANEL_PLY_UNFIN_UNFIN_750'.freeze
    ...
    PLUGIN_NAME = "Euro Cabinet Generator"
...
def cabinet_group?(entity)
    ...
    def load_selected_into_dialog
      ...
      unless grp && grp.valid?
        UI.messagebox("Select a cabinet group generated by Euro Cabinet Generator, then try again.")
...
if type == "Cubbies"
  ...
  params[:door_count] = 0
      ...
      label = grp.name.to_s.strip.empty? ? "Euro Cabinet" : grp.name.to_s.strip
...
when "hinge"
    ...
    def build_cabinet(model, params, target_group: nil, operation_name: nil)
      ...
      op_name = operation_name || (target_group ? "Edit Euro Cabinet" : "Generate Euro Cabinet")
      ...
      base_name = "Euro Cabinet - #{type}" if base_name.empty?
...
write_cabinet_attributes(root, params)
    ...
    def self.show_dialog(edit_selected: false)
      ...
      @dialog ||= UI::HtmlDialog.new(
        dialog_title: "Euro Cabinet Generator",
...
@dialog.add_action_callback("place") do |_ctx, json|
      ...
      is_ada_sink = (type == "ADA Sink")
          ...
          if target && target.is_a?(Sketchup::Group) && target.valid? && cabinet_group?(target)
            root = build_cabinet(model, merged, target_group: target, operation_name: "Edit Euro Cabinet")
          else
            root = build_cabinet(model, merged, target_group: nil, operation_name: "Generate Euro Cabinet")
    ...
    begin
      ...
      def self.show_dialog(edit_selected: false)
        if respond_to?(:_show_dialog_impl)
          ...
          UI.messagebox("Euro Cabinet Generator loaded, but the dialog implementation was not found. Please reinstall the plugin file.")
          nil
        end
      end
    ...
    unless respond_to?(:show_dialog)
      def show_dialog(*args, **kwargs)
        if defined?(SkilledServices::EuroCabinetGenerator) && SkilledServices::EuroCabinetGenerator.respond_to?(:show_dialog)
          ...
          UI.messagebox("Euro Cabinet Generator is not loaded.")
        end
      end
    end
  end
